...
完整的任务 YAML 字段参考（与 `Chemdah/core/quest` 下示例结构一致）。入门说明见 [任务结构](./quest/main)；`addon` 分项见 [任务组件](./quest/addon/control) 等章节；`agent` 见 [代理脚本](./quest/agent/quest)；条目 `condition` / `goal` 见 [条目类型](./quest/task/type)。

<Callout type="info">

条目类型字段名为 **`objective`**（不是 `type`）。游戏中可用 `/chemdah objective <关键词>` 查看已注册类型及条件键；附属插件自定义类型见 [开发 · 自定义目标](./dev/objective)。
...
__option__:
  ...
  # 默认 false；未写时可用 record-completed 作为后备
  record-task-completed: false

  # 兼容旧配置的字段
  # 同时作为 record-quest-completed / record-task-completed 的后备值
...
quest_id:
  ...
  # 是否记录条目完成时间（默认同 __option__，通常 false）
  record-task-completed: false
  ...
  task:
    ...
    task_id:
      ...
      # 条目类型（必填，键名 objective）
      # /chemdah objective <关键词> 查看类型、condition / goal 键
      # 常见类型：
      #   block break          破坏方块
      #   block place          放置方块
      #   player kill          击杀实体
      ...
      condition:
        material: iron_ore
        # 可选：condition.$ 为 Kether 脚本条件（与具体 objective 的键并列）
...
| 主题 | 链接 |
| --- | --- |
| 最小示例与 meta | [任务结构](./quest/main) |
| control / track / ui 等细节 | [任务组件](./quest/addon/control)（侧栏「任务组件」） |
| agent 脚本与 @restrict | [代理脚本](./quest/agent/quest) |
| 各 objective 的 condition | [条目类型](./quest/task/type) |
| 数据隔离与库表 | [开发 · 自定义数据库](./dev/database)、[存档键映射](./dev/persistence-key-codecs) |
